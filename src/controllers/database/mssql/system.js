...
module.exports = (() => {
    ...
    const all = async () => {
        client = client || await conection.connectDB();
        const res = await client.request().query(`select * from ${conection.schema}.SYSTEM`);
    ...
    const get = async (id) => {
        ...
        const res = await request.query(`select * from ${conection.schema}.SYSTEM where id = @id `);
    ...
    const maintance = async (system, val) => {
        ...
        request.query(`UPDATE  ${conection.schema}.SYSTEM  SET MAINTANCE = ${val} WHERE ID = '${system}'`).then((res) => {
            return res;
        }).catch((err) => {
            console.log(err);
        });
    ...
    const update = async (req) => {
        client = client || await conection.connectDB();
        let request = await client.request();
        let queryString = updateUtils.updatetQueryBuilder(`${conection.schema}.SYSTEM`, req.body, true).string;
        try {
    ...
    const remove = async (id) => {
        client = client || await conection.connectDB();
        const res = await client.query(`delete FROM ${conection.schema}.SYSTEM where id = '${id}'`);
